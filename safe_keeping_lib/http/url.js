HarnessInstance.Http.UrlPattern = (function () {
    //adapted from -> https://github.com/snd/url-pattern/blob/master/src/url-pattern.js
    ...
    var UrlMatcher = {
        ...
        newPattern: function (arg, separator) {
            ...
            pattern = Object.create(UrlMatcher.PatternPrototype);
            pattern.isRegex = isRegex;
            pattern.regex = isRegex ? arg : (regexString = UrlMatcher.toRegexString(arg, separator), new RegExp(regexString));
            if (!isRegex) {
                pattern.names = UrlMatcher.getNames(arg, separator);
            }
            return pattern;
        },
        ...
        getNames: function (arg, separator) {
            ...
            escapedSeparator = UrlMatcher.escapeForRegex(separator);
        ...
        toRegexString: function (string, separator) {
            ...
            stringWithEscapedSeparators = UrlMatcher.escapeSeparators(string, separator);
            stringWithEscapedSeparators = stringWithEscapedSeparators.replace(/\((.*?)\)/g, '(?:$1)?').replace(/\*/g, '(.*?)');
            escapedSeparator = UrlMatcher.escapeForRegex(separator);
            UrlMatcher.getNames(string, separator).forEach(function (name) {
